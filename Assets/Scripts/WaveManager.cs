...
public class WaveManager : MonoBehaviour {
	...
	void Update () {
        transform.localPosition = Vector3.MoveTowards(transform.localPosition, posicaoDoPlaneta.localPosition, shootSpeed * Time.deltaTime * 1.4f);
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        { 
            if (other.GetComponent<SpriteRenderer>().color == gameObject.GetComponent<SpriteRenderer>().color)
            {
                Destroy(gameObject);
